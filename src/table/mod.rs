...
pub struct Table {
    ...
    data: Arc<RwLock<HashMap<Box<dyn db_keys::KeyTrait>, Arc<RwLock<BTreeMap<Box<dyn db_keys::KeyTrait>, Value>>>>>>,
}
...
impl Table {
    ...
    pub async fn new(info: &Value) -> Result<Self, errors::DbError> {
        ...
        if let Ok(data) = output.load(info) {
            output.data = data;
        }
        Ok(output)
    }

    pub async fn load(&self, info: &Value) -> Result<HashMap<Box<dyn db_keys::KeyTrait>, Arc<RwLock<BTreeMap<Box<dyn db_keys::KeyTrait>, Value>>>>, errors::DbError> {
        ...
        let Value::Array(data) = data;
        let mut output: HashMap<Box<dyn db_keys::KeyTrait>, Arc<RwLock<BTreeMap<Box<dyn db_keys::KeyTrait>, Value>>>> = HashMap::new();
    ...
    pub async fn update_document(&self, info: Value) -> Result<Value, errors::DbError> {
        ...
        let inner_guard = outer_guard.deref().get(&pk);
        if inner_guard.is_none() {return Ok(NULL_VAL);}
        let mut inner_guard = inner_guard.unwrap().read().await;
        let mut output = inner_guard.get_mut(&sk).unwrap().as_object_mut().unwrap();
